...
def minVal(colorsList):
    minRed = 0
    minGreen = 0
    minBlue = 0
    for handful in colorsList:
        if handful['red'] > minRed:
            minRed = handful['red']
        if handful['blue'] > minBlue:
            minBlue = handful['blue']
        if handful['green'] > minGreen:
            minGreen = handful['green']
...
def runFunction(filename):
    ...
    for game, handfuls in inputdict.items():
        possiblegame = minVal(handfuls)
        possiblegames.append(possiblegame)
    return sum(possiblegames)

if __name__ == "__main__":
    print(runFunction('input2.txt'))